...
/**
 * Global QueryClient pre web app
 * Konfigurácia:
 * - staleTime: 30s - dáta sú považované za "fresh" 30 sekúnd
 * - gcTime: 10 min - garbage collection cache po 10 minútach neaktivity
 * - retry: 2x - automatický retry pre failed requests
 * - refetchOnWindowFocus: true - refetch pri focus (web špecifické)
...
export const queryClient = new QueryClient({
  defaultOptions: {
    queries: {
      ...
      // Refetch behavior
      refetchOnWindowFocus: true, // Web: refetch pri návrate na tab
      refetchOnMount: true,
...
export const queryKeys = {
  ...
  // Audit log
  auditLog: (householdId: string) => ['audit-log', householdId] as const,
} as const;
