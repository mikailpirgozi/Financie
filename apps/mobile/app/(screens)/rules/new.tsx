...
export default function NewRuleScreen() {
  ...
  const loadCategories = async () => {
    try {
      ...
      if (categoriesData.length > 0 && !targetCategoryId) {
        setTargetCategoryId(categoriesData[0].id);
      }
    } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      showToast(
        error instanceof Error ? error.message : 'Nepodarilo sa vytvoriť pravidlo',
        'error'
      );
              ...
              {appliesToOptions.map((option) => (
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    setTargetCategoryId(''); // Reset category when changing type
