...
import { TrendingDown, TrendingUp, Wallet, Calendar, RefreshCw } from 'lucide-react-native';
import { useTheme } from '../../contexts';

interface IncomeHeroCardProps {
  totalIncome: number;
  incomeCount: number;
...
export function IncomeHeroCard({
  totalIncome,
  incomeCount,
  recurringCount = 0,
  percentChange,
  ...
  const getTrendIcon = () => {
    ...
    // For income, going up is positive (more income)
    if (percentChange > 0) {
  ...
  return (
    ...
    >
      ...
      <View style={styles.statsRow}>
        ...
        >
          <Wallet size={14} color={colors.income} />
          <Text style={[styles.statText, { color: colors.income }]}>
            {incomeCount} {incomeCount === 1 ? 'prijem' : incomeCount < 5 ? 'prijmy' : 'prijmov'}
          </Text>
        </View>
        ...
        {incomeCount > 0 && (
          ...
          >
            <Text style={[styles.statText, { color: colors.textSecondary }]}>
              Ø {formatCurrency(totalIncome / incomeCount)}
            </Text>
          </View>
        )}
      </View>
