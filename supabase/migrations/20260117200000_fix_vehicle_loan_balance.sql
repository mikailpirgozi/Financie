-- ============================================
-- FIX VEHICLE LOAN BALANCE CALCULATION
-- Oprava výpočtu zostatku úverov na vozidlá
-- ============================================

-- Problém: Pôvodný view sčítaval principal_balance_after zo všetkých
-- nezaplatených splátok, čo dávalo nesprávne obrovské čísla.
--
-- Správny výpočet: Zostatok je principal_balance_after z poslednej
-- ZAPLATENEJ splátky, alebo pôvodná istina ak žiadna splátka nie je zaplatená.
...
WITH vehicle_loans AS (
  SELECT 
    ...
    COALESCE(SUM(
      -- Total paid: sum of total_due from paid installments
      (SELECT COALESCE(SUM(total_due), 0) FROM loan_schedules WHERE loan_id = l.id AND status = 'paid')
    ), 0) as total_loan_paid,
    COALESCE(SUM(
      -- Current balance: principal_balance_after from last paid installment,
      -- or original principal if no installments paid yet
      COALESCE(
  ...
  FROM assets a
  LEFT JOIN loans l ON l.linked_asset_id = a.id AND l.status = 'active'
...
COMMENT ON VIEW vehicle_tco_summary IS 'Prehľad vozidiel s agregovanými nákladmi (TCO) a stavmi dokumentov - opravený výpočet zostatku úverov';
