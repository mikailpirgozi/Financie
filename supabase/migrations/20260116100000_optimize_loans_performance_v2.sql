-- =====================================================
-- LOANS PERFORMANCE OPTIMIZATION V2
-- =====================================================
-- This migration optimizes the loans query to use a single
-- JOIN with the loan_metrics materialized view, replacing
-- 5 separate queries with 1.
-- =====================================================

-- =====================================================
-- STEP 1: Update loan_metrics to include more details in next_installment
-- =====================================================

-- Drop existing triggers first (they will block the view refresh)
...
-- Drop dependent views first (mv_household_dashboard_summary depends on loan_metrics)
DROP MATERIALIZED VIEW IF EXISTS mv_household_dashboard_summary CASCADE;

-- Drop and recreate the materialized view with enhanced next_installment
...
-- =====================================================
-- STEP 2: Create safe refresh function (non-blocking)
-- =====================================================

CREATE OR REPLACE FUNCTION refresh_loan_metrics_safe()
RETURNS void AS $$
BEGIN
  -- Use CONCURRENTLY to avoid blocking reads
  -- This requires the unique index on loan_id
  REFRESH MATERIALIZED VIEW CONCURRENTLY loan_metrics;
EXCEPTION
  WHEN OTHERS THEN
    -- If concurrent refresh fails, try regular refresh
    -- This can happen if the view is being refreshed elsewhere
...
GRANT EXECUTE ON FUNCTION refresh_loan_metrics_safe() TO authenticated;

-- =====================================================
-- STEP 3: Create optimized get_loans_with_metrics RPC
-- =====================================================
...
-- =====================================================
-- STEP 4: Create optional scheduled refresh (pg_cron)
-- =====================================================
-- Note: This requires pg_cron extension to be enabled
-- Uncomment if you want automatic background refresh
...
-- =====================================================
-- STEP 6: Recreate mv_household_dashboard_summary
-- (depends on loan_metrics, was dropped at start)
-- =====================================================
...
-- =====================================================
-- NOTES:
-- - The trigger-based refresh has been removed to avoid
--   blocking writes during bulk operations
-- - Use refresh_loan_metrics_safe() for on-demand refresh
-- - Consider enabling pg_cron for background refresh
-- - Data staleness of up to 5 minutes is acceptable for
--   dashboard displays
-- =====================================================
